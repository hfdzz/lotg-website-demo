<article class="result-card tree-node" style="margin-left: {{ $node['depth'] * 1.25 }}rem;">
    ...
    <h3><a href="{{ route('admin.nodes.edit', ['edition' => request()->route('edition'), 'law' => $law, 'node' => $node['id']]) }}">{{ $node['title'] }}</a></h3>
    <p class="law-meta">
    ...
    <p class="law-meta"><a class="result-link" href="{{ route('admin.nodes.edit', ['edition' => request()->route('edition'), 'law' => $law, 'node' => $node['id']]) }}">Edit this node</a></p>
</article>
